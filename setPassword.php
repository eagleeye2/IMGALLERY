...
$name= $_SESSION['user'];
	...
	if(isset($_POST['submit']))
	{
		...
		$cpass=$_POST['confirm'];
		 ...
		 $sql="update signin set password='$pass' where username='$name'"; 
		 $rs=mysql_query($sql);
		
		$sql="update signup set password='$pass' where username='$name'"; 
		 $rs=mysql_query($sql);
...
<form id="f2" method="post" action="" onSubmit="return doValidate();">
	...
	<div id="username" ><input type="text" name="new" placeholder="enter new password" size="30" style="border:2px solid #726E6D;     background-color:#F7F7F7;color:#726E6D;height:25px;" id="username1" /><label id="catname" style="color:#FF0000"></label> </div>
	
	<div id="password">
      <input name="confirm" type="password" style="border:2px solid #726E6D;background-color:#F7F7F7;color:#726E6D;height:25px;" size="30"  placeholder="Re-enter new password" id="password1" /><label id="catname1" style="color:#FF0000"></label>
    </div>
	
	
	
	<div id="btn">
    	<input type="submit" name="submit" value="save" style="background-color:#726E6D;color:#FFF;height:25px;width:70px; border-radius:5px 5px 5px 5px; border:2px solid #999999; font-family:Calibri;font-size:16px;" />
	</div>
